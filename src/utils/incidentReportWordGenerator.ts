...
// 處理複選框欄位，☐字體大小12pt，☑正常字體
const processCheckbox = (obj: any, key: string, fontSize = 12): string => {
  if (obj && obj[key]) return '☑';
  return `<span style="font-size:${fontSize}pt">☐</span>`;
};
...
export const convertIncidentReportToTemplateData = (
  ...
  data['事故性質_跌倒'] = report.incident_type === '跌倒' ? '☑' : `<span style="font-size:12pt">☐</span>`;
  data['事故性質_其他'] = report.incident_type === '其他' ? '☑' : `<span style="font-size:12pt">☐</span>`;
  ...
  Object.entries(locationMap).forEach(([key, placeholder]) => {
    data[`地點_${placeholder}`] = report.location === key ? '☑' : `<span style="font-size:12pt">☐</span>`;
  });
  ...
  Object.entries(activityMap).forEach(([key, placeholder]) => {
    data[`活動_${placeholder}`] = report.patient_activity === key ? '☑' : `<span style="font-size:12pt">☐</span>`;
  });
  ...
  data['意識_清醒'] = report.consciousness_level === '清醒' ? '☑' : `<span style="font-size:12pt">☐</span>`;
  data['意識_混亂'] = report.consciousness_level === '混亂' ? '☑' : `<span style="font-size:12pt">☐</span>`;
  data['意識_昏迷'] = report.consciousness_level === '昏迷' ? '☑' : `<span style="font-size:12pt">☐</span>`;

  if (report.limb_movement) {
    const lm = report.limb_movement;
    data['四肢活動_正常'] = lm.status === '正常' ? '☑' : `<span style="font-size:12pt">☐</span>`;
    data['四肢活動_異常'] = lm.status === '異常' ? '☑' : `<span style="font-size:12pt">☐</span>`;
    data['四肢活動_詳情'] = lm.details || '';

    const abnormalLimbs = lm.abnormal_limbs || [];
    data['四肢_左上肢'] = abnormalLimbs.includes('左上肢') ? '☑' : `<span style="font-size:12pt">☐</span>`;
    data['四肢_右上肢'] = abnormalLimbs.includes('右上肢') ? '☑' : `<span style="font-size:12pt">☐</span>`;
    data['四肢_左下肢'] = abnormalLimbs.includes('左下肢') ? '☑' : `<span style="font-size:12pt">☐</span>`;
    data['四肢_右下肢'] = abnormalLimbs.includes('右下肢') ? '☑' : `<span style="font-size:12pt">☐</span>`;
  } else {
    data['四肢活動_正常'] = `<span style="font-size:12pt">☐</span>`;
    data['四肢活動_異常'] = `<span style="font-size:12pt">☐</span>`;
    data['四肢活動_詳情'] = '';
    data['四肢_左上肢'] = `<span style="font-size:12pt">☐</span>`;
    data['四肢_右上肢'] = `<span style="font-size:12pt">☐</span>`;
    data['四肢_左下肢'] = `<span style="font-size:12pt">☐</span>`;
    data['四肢_右下肢'] = `<span style="font-size:12pt">☐</span>`;
  }
  ...
  data['醫療安排_急症室'] = report.medical_arrangement === '急症室' ? '☑' : `<span style="font-size:12pt">☐</span>`;
  data['醫療安排_門診'] = report.medical_arrangement === '門診' ? '☑' : `<span style="font-size:12pt">☐</span>`;
  data['醫療安排_醫生到診'] = report.medical_arrangement === '醫生到診' ? '☑' : `<span style="font-size:12pt">☐</span>`;
  data['醫療安排_沒有送院'] = report.medical_arrangement === '沒有送院' ? '☑' : `<span style="font-size:12pt">☐</span>`;
  ...
  data['家屬關係_保證人'] = report.family_relationship === '保證人' ? '☑' : `<span style="font-size:12pt">☐</span>`;
  data['家屬關係_監護人'] = report.family_relationship === '監護人' ? '☑' : `<span style="font-size:12pt">☐</span>`;
  data['家屬關係_家人'] = report.family_relationship === '家人' ? '☑' : `<span style="font-size:12pt">☐</span>`;
  data['家屬關係_其他'] = report.family_relationship === '其他' ? '☑' : `<span style="font-size:12pt">☐</span>`;
  ...
  data['社署報告_需要'] = submitToSocialWelfare ? '☑' : `<span style="font-size:12pt">☐</span>`;
  data['社署報告_不需要'] = submitToSocialWelfare ? `<span style="font-size:12pt">☐</span>` : '☑';

  const submitToHeadquarters = report.submit_to_headquarters || report.submit_to_headquarters_flag;
  data['總部報告_需要'] = submitToHeadquarters ? '☑' : `<span style="font-size:12pt">☐</span>`;
  data['總部報告_不需要'] = submitToHeadquarters ? `<span style="font-size:12pt">☐</span>` : '☑';

  data['呈報社會福利署'] = submitToSocialWelfare ? '☑' : `<span style="font-size:12pt">☐</span>`;
  data['呈報總部'] = submitToHeadquarters ? '☑' : `<span style="font-size:12pt">☐</span>`;
